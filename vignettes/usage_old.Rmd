...
```
```{r}
heatmap_cmp(cmp, what ="changes", only_feat = c("Ms4a1", "Ccl19", "Ccl25", "Rag1", "Dntt", "Pcna", "Mki67", "Ccl22", "Spib", "Cd19", "Ighd", "Cd3e", "Cd3d", "Ccr6", "Dcn", "Ccr9", "Cxcl12", "Aire", "fezf2", "Krt14", "H2-Aa", "S1pr1"), hclust_method = "single")
```
...
um <- um$layout
colnames(um) <- c("x", "y")
ggplot(as.data.frame(um), aes(x=x, y=y, label=rownames(um))) + geom_point() + ggrepel::geom_text_repel(size=2)
```
