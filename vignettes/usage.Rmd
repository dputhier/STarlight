...
```{r}
goi <- "Cd19"
st_obj_1$has_signal <- as.factor(ifelse(bin_mat(st_obj_1)[,"all_genes"] > 20,
            1, 
            0))
goi_mat <- bin_mat(st_obj_1, as_factor = TRUE)[,c("bin_x", "bin_y", goi)]
goi_mat <- goi_mat[goi_mat[[goi]] > 0, ]
ggplot(cbind(bin_mat(st_obj_1, as_factor = TRUE), st_obj_1$has_signal), mapping = aes(x=bin_x, y=bin_y, color=st_obj_1$has_signal)) +
geom_point() +
geom_point(data=goi_mat, 
    mapping=aes(x=bin_x, y=bin_y),
