...
```{r loadfile}
st_obj <- load_spatial("~/Downloads/Xenium_V1_FF_Mouse_Brain_Coronal_Subset_CTX_HP_outs/", 
                         method = "xenium",
                         verbose = FALSE, 
                         bin_size = 25)
...
```{r}
spatial_image(st_obj, features = hc_st$tree_classes[["MOD18"]])
```

```{r}
spatial_image(st_obj, features = hc_st$tree_classes[["MOD06"]])
```
...
We may use the K-Ripley's function to assess the level of clustering of each molecules in the two regions. Not that this step may be quite long as the density of molecule increases. Here we will thus just focus on representative genes.


```{r compute_k_ripley}
st_obj <- compute_k_ripley(st_obj, rmax = 200, verbose = FALSE)
```
...
```
```{r}
spatial_image(st_obj, features = c("Chat", "Spag16", "Tacr1", "Sncg"))
```
...
First we prepare an stgrid object containing another region of the tissue. We will compare the two subregions (but two datasets/experiments could be also compared).

```{r stcompr}
x_bins <-  bin_x(st_obj)[61:101]
y_bins <-  bin_y(st_obj)[101:nbin_y(st_obj)]
st_obj_r2 <- st_obj[x_bins, y_bins]
```

Below is the density of molecules counts in this regions:

```{r all_genes}
spatial_image(st_obj_r1, feature="count_sum") + Seurat::NoLegend() + 
spatial_image(st_obj_r2, feature="count_sum") 
```
Now we can create an STCompR object using the stcompr() function. 

```{r stcompr_create}
cmp <- stcompr(st_obj_r2, st_obj_r1, name_1="region_1", name_2="region_2")
```
...
```
We may check these molecules using the `cmp_images()` function:

```{r}
markers <- cmp@stat_test %>% 
              dplyr::filter(abs(log2_ratio) > 5) %>% 
              dplyr::filter(-log10(p_values) > 200) %>% rownames()

cmp_images(st_obj_r1,
           st_obj_r2,
           feat_list = markers)
```
...
```{r compute_k_ripley}
st_obj_r1 <- compute_k_ripley(st_obj_r1, rmax = 200, verbose = FALSE)
st_obj_r2 <- compute_k_ripley(st_obj_r2, rmax = 200, verbose = FALSE)
```

Plotting the results highlight the clustering levels of molecules in both regions:

```{r plot_rip_k}
plot_rip_k(st_obj_r1, size=2) +
plot_rip_k(st_obj_r2, size=2)
```

We can confirm that these molecules are highly packed in the first and/or the second region.

```{r ripley_image}
cmp_images(st_obj_r1,
           feat_list = head(order_feat_by_ripley(st_obj_r1)))
```
```{r}
cmp_images(st_obj_r2,
           feat_list = head(order_feat_by_ripley(st_obj_r2)))
```
...
As an example we can clearly see that  Kctd8, Necab2, Nrp2 or Chat molecules relocate toward Nwd2. indeed, a negative value indictae 

```{r heatmap_cmp_img}
cmp_images(st_obj_r2, st_obj_r1, feat_list = c("Nwd2",
                                               "Kctd8",
                                               "Ccn2",
                                               "Cplx3"), 
...
cell_coord <- data.frame(GetTissueCoordinates(nano.obj)[,1:2],
                         stats::setNames(nano.obj$predicted.annotation.l1, NULL))
...
```{r}
cell_1 <- load_spatial("/Users/puthier/Downloads/Lung5_Rep1_nanostring.txt", 
                         method = "coordinates",
                         verbose = FALSE, 
                         control = NULL, 
                       bin_size = 100)
...
cmp_images(foi_1, foi_2, feat_list=c("Myofibroblast", "Fibroblast", "CD8 T"), names = c("foi_1", "foi_2"))
