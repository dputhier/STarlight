...
```{r zoom}
x_bins <-  bin_x(st_obj)[181:length(bin_x(st_obj))]
y_bins <-  bin_y(st_obj)[101:length(bin_y(st_obj))]
...
```{r all_genes}
spatial_image(st_obj_r1, feature="sum_of_cts") + Seurat::NoLegend() + 
  spatial_image(st_obj_r2, feature="sum_of_cts") 
```
Now we can create an STCompR object using the stcompr() function. 

```{r stcompr_create}
cmp <- stcompr(st_obj_r1, st_obj_r2, name_1="region_1", name_2="region_2")
```
...
```{r heatmap_cmp_img}
cmp_images(st_obj_r1, st_obj_r2, feat_list = c("Nwd2",
                                             "Kctd8",
                                             "Necab2",
                                             "Nrp2", "Chat"), names=c("Region_1", "Region_2"))
```
