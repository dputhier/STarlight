...
Spatial transcriptomics (ST) enables the spatially resolved measurement of gene expression within tissue sections, providing valuable insights into the spatial organization of biological processes. Assigning RNA molecules to cells using a segmentation process is generally the first step in the analysis. This approach can be tricky for a number of reasons, including high cell density, the absence or poor quality of surface labelling, etc. Thus cell-segmentation is frequently performed in selected and very sparse areas which impedes having a global view of the results at the tissue level.
The `STarlight` package is intended to perform analysis of imaging-based ST (*e.g.* Merscope, Xenium or CosMx) and bypasses cell-segmentation by applying a grid-based approach. This complementary approach allows to get a global view of the whole experiment.
The `load_spatial()`is the entry point of the `STarlight` package It will load the molecule coordinates and create a 2D binned grid with default size 25µm.
...
Here we load the data considering that the Xenium dataset is a simple csv flat file containing coordinates. If the full result directory is available you may use method="xenium" and provide the path to the directory as argument to 'path'.
...
st_obj
```
Here are the slots contained in the objects.

```{r slots, echo = TRUE, eval=TRUE, warning=FALSE, results='hide'}
slotNames(st_obj)
```
As shown below, the STGrid object supports implements numerous methods. 
