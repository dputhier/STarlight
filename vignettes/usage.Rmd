...
## The gene_contrast() function

This function can be used to compare the log2 ratio between two genes across x/y bins. Several pair of genes can be provided to the function.

```{r gene_contrast, warning=FALSE, results='hide', echo = TRUE, eval=TRUE, fig.height=3, fig.width=5}
gene_contrast(
  st_obj,
  feat_list_1 = c("Nwd2",
                  "Kctd8",
  ...
  feat_list_2 = c("Igf1",
                  ...
                  "Hat1"),
  centered = TRUE, 
  trim_ratio = 0.025) + lgd_theme

```

## The multi_gene_contrast() function

This function performs a contrast analysis between multiple features (genes) using an STGrid as input. It binarizes the gene expression based on a user-defined threshold across x/y bins. For each bin a binary vector is computed and encoded as a color (e.g 000, none of 3 genes above the threshold; 100, gene 1 only above the threshold; 110, gene 1 and 2 above the threshold...). The number of required colors raise rapidly (2^(length(feat_list))) as the number of features increases. So it it advised not to go beyond 4 features/genes.

```{r multi_gene_contrast, warning=FALSE, results='hide', echo = TRUE, eval=TRUE, fig.height=3, fig.width=4}
multi_gene_contrast(
  st_obj,
  feat_list = c("Ebf3",
                "Kctd8",
...
Cell populations expressing a specific marker may be found more or less clustered in the tissue. The  bins containing the corresponding molecules may appear as connected components that can be captured using the  *connected_components()* function. The connected components are stored in the *meta* slot of the STGrid object and named with original feature name and a "_cpt" suffix. Here we will search for connected components with displaying at least 1 count for Cabp7 filtering out those with surface lower than 9 bins. We will also add a hull around the  connected components.

```{r}
set_verb_level(1)
library(patchwork)
st_obj <- connected_components(st_obj, feat_list = "Nwd2", threshold = 1, min_size = 8)
head(meta(st_obj), 3)
nb_cc <- length(unique(st_obj$Nwd2_cpt))
p1 <- spatial_image(st_obj, features = "Nwd2_cpt", 
                    as_factor = TRUE, 
                    colors = c("black", rainbow(nb_cc - 1))) + lgd_theme
p2 <- p1 + create_hull(st_obj, feat = "Nwd2_cpt")  + lgd_theme
p1 + p2
```
We can next  find the neighbors of connected componed with the objective to more clearly highlight the component of interest.

```{r}
st_obj <- cc_neighborhood(st_obj, feat = "Nwd2_cpt")
head(meta(st_obj), 3)
p1 + create_hull(st_obj, feat = "Nwd2_cpt_ngb")  + lgd_theme
```

And use this hull as a footprint of Nwd2 gene expression when analysis Cabp7.

```{r}
spatial_image(st_obj, "Cabp7") + create_hull(st_obj, feat = "Nwd2_cpt_ngb", linewidth = 1, color="white") + lgd_theme
```
...
spatial_image(st_obj, features = meta_names(st_obj)[2:21], ncol = 4) + lgd_theme
```
...
spatial_image(st_obj, features = c("Chat", "Spag16", "Tacr1", "Calb2"), ncol = 2) + lgd_theme
```
...
```{r compare_count_distribution, warning=FALSE, results='hide', echo = TRUE, eval=TRUE, fig.height=3, fig.width=3}
