...
The `load_spatial()` function is the entry point of the `STarlight` package. It will load molecule coordinates and create a 2D binned grid (default size of 25µm). Since we are working with a csv flat file containing x/y coordinates and gene names we do not need to specify the argument method. If you have access to the full Xenium directory, you can use the arguments method="xenium" and path="path/to/directory" to load the data. 
