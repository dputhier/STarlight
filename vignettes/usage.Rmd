...
plot_rip_k(compute_k_ripley(st_obj[gene_list, ], rmax = 200, verbose = FALSE))
```
...
```{r heatmap_cmp}
heatmap_cmp(cmp, filter = 1.2, size = 6)
```
...
markers <- c("Nwd2",
  ...
  "Ccn2",
  "Cplx3")
cmp_images(re_bin(st_obj_r2[markers,], 10), 
           re_bin(st_obj_r1[markers,], 10), feat_list = c("Nwd2",
                                               "Kctd8",
                                               "Ccn2",
                                               "Cplx3"), 
...
u <- p$layout
colnames(u) <- c("x", "y")
u$class <- unlist(hc_st$tree_classes)
ggplot(as.data.frame(u), aes(x=x, y=y, label=rownames(u))) + geom_point() + ggrepel::geom_text_repel(size=3)
```
