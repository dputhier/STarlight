...
Spatial transcriptomics (ST) enables the spatially resolved measurement of gene expression within tissue sections, providing valuable insights into the spatial organization of biological processes. Assigning RNA molecules to cells using a segmentation process is generally the first step in the analysis. This approach can be tricky for a number of reasons, including high cell density, the absence or poor quality of surface labelling, etc. `STarlight` package is intended to propose an alternative to cell-segmentation based analysis of imaging-based ST (*e.g.* Merscope, Xenium or CosMx) for user working with gene-panel markers. To this aim, `STarlight` proposes an analysis based on fixed-size tiling (sometimes referred as 'rasterization'). The `load_spatial()`is the entry point of the `STarlight` package It will load the molecule coordinates and create a 2D binned grid with default size 25µm.
