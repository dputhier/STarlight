...
# First prepare a general ggplot theme for this presentation
gg_theme <- theme(
...
# Call the spatial_image() function
 p <- spatial_image(
  st_obj,
  feat = "Ano1",
  grid = 20,
...
```{r gene_contrast, warning=FALSE, results='hide', echo = TRUE, eval=TRUE, fig.height=2, fig.width=5}
gene_contrast(
  st_obj,
  feat_list_1 = c("Nwd2",
                  "Kctd8",
...
```{r multi_gene_contrast, warning=FALSE, results='hide', echo = TRUE, eval=TRUE, fig.height=2, fig.width=4}
multi_gene_contrast(
  st_obj,
  feat_list = c("Ebf3",
                "Kctd8",
...
```{r connected_components, warning=FALSE, results='hide', echo = TRUE, eval=TRUE, fig.height=4, fig.width=7}
st_obj <- connected_components(st_obj, feat_list = "Nwd2", 
                               threshold = 1, min_size = 8)
nb_cc <- length(unique(st_obj$Nwd2_cpt))
p1 <- spatial_image(st_obj, features = "Nwd2_cpt", 
                    as_factor = TRUE, 
                    colors = c("black", rainbow(nb_cc - 1))) + gg_theme
p2 <- p1 + create_hull(st_obj, feat = "Nwd2_cpt")  + gg_theme
p1 + p2
```
...
head(meta(st_obj), 3)
p1 + create_hull(st_obj, feat = "Nwd2_cpt_ngb")  + gg_theme
```

And use this hull as a footprint of Nwd2 gene expression when analysing Cabp7.

```{r Cabp7, warning=FALSE, results='hide', echo = TRUE, eval=TRUE, fig.height=2, fig.width=4}
spatial_image(st_obj, "Cabp7") + create_hull(st_obj, 
                                             feat = "Nwd2_cpt_ngb", 
                                             linewidth = .5, color="white") + gg_theme
```
...
spatial_image(st_obj, features = meta_names(st_obj)[2:21], ncol = 4) + gg_theme
```
...
spatial_image(st_obj, features = hc_st$tree_classes[["MOD07"]], ncol = 3) + gg_theme
```
...
plot_rip_k(compute_k_ripley(st_obj, rmax = 80, verbose = FALSE)) + gg_theme
```
...
spatial_image(st_obj, features = c("Chat", "Spag16", "Tacr1", "Calb2"), ncol = 2) + gg_theme
```
