...
library(patchwork)
library(ggplot2)
st_obj_1 <- load_spatial("~/Downloads/Xenium_V1_FF_Mouse_Brain_Coronal_Subset_CTX_HP_outs/", method = "xenium")
```
# The STGrid object
...
spatial_image(re_bin(st_obj_2["Ano1",], bin_size = 5), gene="Ano1")
```
...
```{r compute_k_ripley}
st_obj_2 <- compute_k_ripley(st_obj_2, rmax = 200)
st_obj_3 <- compute_k_ripley(st_obj_3, rmax = 200)
```
